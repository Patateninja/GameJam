...
#pragma endregion
	...
	{
		// Mise � jour de la direction du joueur selon l'angle de rotation
		...
		direction.y = -std::cos(angle * DEG2RAD); // Le signe n�gatif pour l'axe Y (sinon l'orientation est invers�e)
	}
...
#pragma region SoloInput
		...
		if (isMovingUpRight)
		{
			...
			rotation -= rotationSpeed * getDeltaTime();  // Tourner � gauche
		}
		...
		{
			position += direction * moveSpeed * getDeltaTime();
			rotation += rotationSpeed * getDeltaTime();  // Tourner � droite
		}
...
#pragma region DuoInput
		...
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Scancode::W) &&
			...
			!sf::Keyboard::isKeyPressed(sf::Keyboard::Scancode::Up))
		{
			rotation -= rotationSpeed * getDeltaTime();  // Rotation lente � gauche
		}
		...
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Scancode::S) &&
			...
			!sf::Keyboard::isKeyPressed(sf::Keyboard::Scancode::Down))
		{
			rotation += rotationSpeed * getDeltaTime();  // Rotation lente � droite
		}
